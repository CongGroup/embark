...
public:
	...
	virtual T find(const T& x);
	virtual T findEQ(const T& x);
	virtual int size();
	virtual void clear();
};
...
template<class Node, class T>
T BinarySearchTree<Node,T>::findEQ(const T& x) {
	Node *w = r;
	while (w != nil) {
		int comp = compare(x, w->x);
		if (comp < 0) {
...
template<class Node, class T>
T BinarySearchTree<Node,T>::find(const T& x) {
	Node *w = r, *z = nil;
	while (w != nil) {
		int comp = compare(x, w->x);
		if (comp < 0) {
...
template<class Node, class T>
bool BinarySearchTree<Node, T>::add(const T& x) {
	Node *p = findLast(x);
	Node *u = new Node;
	u->x = x;
	return addChild(p, u);
...
template<class Node, class T>
bool BinarySearchTree<Node, T>::remove(const T& x) {
	Node *u = findLast(x);
