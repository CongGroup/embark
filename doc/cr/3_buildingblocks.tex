...
Our PrefixMatch encrypts in $< 3\mu$s per value \clan{Update the value} (we evaluate in \S\ref{sec:eval}).
...
\Procedure{EncryptRanges}{$R_1$, $\dots$, $R_n$}
  ...
  \ForAll{sorted values}
    \If{current value is $s_i$}
      \State Ranges.Add($R_i$) 
      \If{$R_i$.priority > TopRange.priority}
