...
\todo{mixture of intervals, ranges and prefixes, clarify, also endpoints}
\todo{and do you tell the size of the endpoint}
...
\item Let $s_i$ and $e_i$ be the endpoints of $P_i$. \comment{$P_i = [s_i, e_i]$}
	...
	\item  Construct non-overlapping intervals $I_0, \dots, I_m$ from each pair of consecutive endpoints. Multiple equal endpoints form a unit interval. For each interval $I_i$, compute $\prefixes(I_i)$, the list of prefixes $P_{i_1}, \dots, P_{i_m}$ that contain   $I_i$. 
