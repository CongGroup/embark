...
read SMTP messages encrypted by KeywordMatch. Application-level gateways are useful for certain address-dependent 
...
rewrite those fields that are in the application-level data, which is encrypted by KeywordMatch.
Therefore, KeywordMatch needs to be tailored for each protocol, which is beyond the scope of this
paper.

Also, \sys does not support transcoders, who perform on-the-fly conversion of application level 
data. Such operations are not supported by KeywordMatch.
