...
Application-level firewalls act as a protocol end point and relay (e.g., an SMTP client/server or a Web proxy agent) \todo{relay what?}. KeywordMatch does not support all protocols. \todo{why??}
\todo{need to say that some app firewalls also support regexp right, and this is not supported}
Application-level gateways are useful for certain address-dependent protocols such as FTP; these do not change the semantics of the application protocol, but carry out mechanical substitution of fields. KeywordMatch does not support all protocols and need to be tailored for each protocol. \todo{what does not support, can you be more specific?}

Also, \sys does not support transcoders, who perform on-the-fly conversion of application level data. KeywordMatch doesn’t support them.
