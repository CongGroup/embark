...
\noindent{\bf Decryption.} PrefixMatch is not reversible. To enable packet decryption, we store the AES encrypted values for the header fields in the IP options header. When the gateway receives a packet to decrypt, it decrypts the values from the options header and restores them.% to the IP or transport header.
