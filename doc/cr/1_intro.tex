...
HTTP Proxy / Cache &
$ \text{URL} \in \text{HTTP Header} \Rightarrow \Match(\Enc(\text{URL}),\ \Enc(\text{HTTP Header}))=1 $
&
...
\parbox {8cm}{
\begin{flalign*}
& \text{URL} \in \text{HTTP Header}\  \Leftrightarrow\ \Match(\Enc(\text{URL}),\ \Enc(\text{HTTP Header})) = 1
\end{flalign*}
} &
Yes & KeywordMatch \\
%
...
%
&
$\text{regexp} \in \text{stream} \quad \Leftrightarrow \quad \Match(\Enc(\text{regexp}),\Enc(\text{stream}))=1$
&
