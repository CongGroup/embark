...
\sys supports the ``textbook'' functionality of almost all middleboxes considered for outsourcing in APLOMB~\cite{aplomb}.
Table~\ref{tbl:mbreqs} lists the middleboxes supported by \sys.  In this paper, we consider the functionality of the middlebox to be the one 
described in textbooks and references\todo{add cite}. We note that there exist versions of these middleboxes with more complex functionality that \sys may not support: we discuss \sys's limitations in \S\ref{s:limitations}.
