...
& {\bf Middlebox}  & {\bf Type } & {\bf Example field}  \\
\hline \hline
\capp &  IP Firewall \S\ref{sec:firewall} & HO &  IP address, port   \\
\capp &  Application Firewall  \S\ref{sec:firewall} & BA & IP address, payload  \\
\capp & NAT  \S\ref{sec:nat} & HO & IP address, port   \\
\capp & IP Forwarding  \S\ref{sec:other_apps}  & HO &  IP address, port \\
...
\end{tabular}
\caption{Middleboxes supported by \sys, shown with the section that discusses them, their type, HO (header only) or BA (bytestream aware), and one or two examples of encrypted fields they operate on (not comprehensive). \label{tab:apps-ops} }
\end{table}
