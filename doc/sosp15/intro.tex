...
\begin{table}[t!]
\centering  
\begin{tabular}{c|p{3.3cm}|p{0.6cm}|p{3cm}}
& {\bf Middlebox}  & {\bf Type } & {\bf Operations used}  \\
\hline \hline
\capp &  IP Firewall \S\ref{sec:firewall} & HO &  range    \\
\capp &  Application Firewall  \S\ref{sec:firewall} & BA & range, keyword  \\
\capp & NAT  \S\ref{sec:nat} & HO & range   \\
\capp & IP Forwarding  \S\ref{sec:other_apps}  & HO &  range \\
\capp & Load Balancer L4 \S\ref{sec:loadb} & HO & range  \\
\capp & Load Balancer L7 \S\ref{sec:loadb} & BA & range  \\
\capp & Web Proxy/Cache  \S\ref{s:proxy} & HO & keyword \\
\capp & Intrusion Detect (IDS) \S\ref{sec:ids} & BA & range, keyword  \\
\capp & Data Exfiltration  \S\ref{sec:ids} & BA & keyword  \\
\capp & VPN Gateway \S\ref{sec:vpn} &  HO & none  \\ 
\end{tabular}
\caption{Middleboxes supported by \sys \todo{Section too} and their type, HO (header only) or BA (bytestream aware) type \todo{finish} of match it needs to perform. Each application is further described in the section specified. \label{tab:apps-ops} }
