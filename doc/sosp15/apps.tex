...
Hence, the IDS is used with the stateful gateway.

\sys's IDS is based on BlindBox~\cite{blindbox}, an IDS which uses searchable encryption to detect malicious signatures within encrypted HTTPS connections.
However, the \sys `outsourcing' setting is a very different use case than what BlindBox was designed for; in adapting BlindBox to \sys, client performance actually improved {\it and} security guarantees became stronger.We briefly discuss BlindBox and the modifications we made to fit our setting.
...
In practice, the sender does not send encrypted tokens for {\it every} byte, but optimizes out some which are redundant or unnecessary (we defer to~\cite{blindbox} for details about these optimizations, and the rest of the design in general).
...
We use the same tokenization scheme  and its optimizations, as well as the `probable cause decryption' model from BlindBox.

The key difference between BlindBox and how \sys implements IDS comes in how rules are generated at the IDS.
