...
Unlike the previously presented middleboxes, devices which perform Intrusion Detection/Prevention (IDS/IPS) inspect the packet payload, not just the IP and TCP/UDP headers.
Appliances which operate on payloads are said to perform ``deep packet inspection'' (DPI); they perform ``stateful DPI'' reconstruct the TCP bytestream at the middlebox and operate over this bytestream, not just each packet individually.
In the case of a proxy, reconstructing the bytestream allows the proxy to store entire images (which would normally be divided across multiple packets) and cache them to speed up client performance. 
...
To support IDSes, however, we must use a modified gateway which supports DPI middleboxes by reconstructing the TCP flow at the middlebox.
As we show in~\S\ref{sec:eval}, the stateful gateway achieves lower throughput than the stateless one.
Fortunately, not all stateful DPI middleboxes require a stateful gateway. 
As we shall see, some HTTP proxies -- proxies for non-pipelined HTTP requests -- are stateful DPI devices, but can be fully supported with a stateless \sys gateway (pipelined HTTP proxies, however, do require a stateful gateway).
