...
and ``cure'' token in the second packet, none of which match "insecure". 
 ...
 The insight is to for the gateway to somehow tie together the last $i$ bytes from the end of a packet to the first $m-i$ bytes from the beginning of the next packet. This is difficult because the gateway cannot order packets. Nevertheless, each packet has a serial number at a fixed offset in the packet, which is easy to read for the gateway. The encryption of the packet's payload margins proceeds as follows:
