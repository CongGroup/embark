...
APLOMB is a system which redirects an enterprise's traffic to the cloud for middlebox processing~\cite{aplomb}.
...
Some middleboxes are `bytestream-aware.' These middleboxes operate over a TCP bytestream -- the concatenation of all {\it payloads} -- and hence must keep copies of the data transmitted for each connection.
...
Because of this reconstruction cost, and the complexity of the computation over the reconstructed data, most bytstream aware middleboxes achieve lower throughput than header-only middleboxes.
...
First, the enterprise traffic must be processed in IPv6 (although the tunnel connecting the enterprise to the cloud may be v4 or v6). Processing IPv6 packets rather than v4 is a requirement of our RangeMatch scheme whose security requires a more than 32 bits to encode encrypted IP addresses. Nevertheless, this is not unusual considering that more and more enterprises are switching to IPv6. Moreover, \sys does not require the client and the server to use IPv6: they can use IPv4 in which case the gateway converts the packet to IPv6 using the Stateless IP/ICMP Translation Algorithm (SIIT)~\cite{SIIT}.
...
After encrypting the header, the gateway encrypts the payload  with a standard cipher such as AES. 
