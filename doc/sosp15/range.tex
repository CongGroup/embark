...
\section{Building block: Range match } \label{sec:range}
...
In this section, for concreteness, we explain our scheme by considering $v$, $e_i$ and $s_i$ as IP addresses (although the scheme can be used for encrypting ports too), the encryptor as the gateway, the party evaluating ``>'' the service provider (SP) because this is how \sys uses the scheme. 

\mypara{Requirements on an encryption scheme}
%
...
\item {\em be deterministic}: To integrate with the NAT, we need to make sure that each value gets assigned consistently  the same encryption.  \label{req:injective}
...
\subsection{Our RangeMatch scheme} 
...
When encrypting IP addresses, we do not want two different IP addresses to map to the same encryption (which would break the NAT). Fortunately, the probability that  two IP addresses get assigned to the same encryption is negligibly low for IPv6 and very low for IPv4.~\todo{do we need to tell them it works for ipv6 only?} The reason is that  each interval of encryptions is large because we distributed the endpoint encryptions evenly and because there is a small number of such endpoints in a realistic setting (e.g., a firewall has less than 100,000 rules).
