...
We adapted all header-only middleboxes {\it without modifying the dataplane functions}, and hence preserving the steady-state performance properties of the existing middleboxes.

Middleboxes which perform {\it header-only} operations and keep no {\it per-flow state} were the easiest to adapt, because they required no modifications at all; these middleboxes were the stateless IP Firewalls and the IP Routing. 
So long as the rules (for the firewalls) or the forwarding table (for the IP router) are encrypted with the range match approach, the existing code bases work normally on encrypted packets.

Middleboxes which perform {\it header-only} operations but which keep some per-flow state required modification to the control logic.
These middleboxes include the NAT, stateful firewalls, and load balancers.
The reason these middleboxes require some modification is because adjustments to the range match scheme or changes to the gateway key $k$ require changes to the per-flow state.
When there is a tree adjustment or a key change, the gateway generates new rulesets and sends these to the middleboxes; this is the only update required for the stateless header-only devices.
Stateful devices, however, enter a second round of update at this point... \todo{Talk to Raluca upstairs}.

Finally, bytestream-aware middleboxes require changes in the dataplane: \sys makes reading the payload directly impossible, and hence these middleboxes must be modified to operate over the metadata channel instead.
