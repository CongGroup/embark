...
However, BlindBox is far from sufficient for our setting because
 ...
 Regarding functionality, BlindBox enables equality-based operations on the encrypted traffic which supports certain DPI devices. However, this excludes middleboxes such as firewalls, proxies, NAT, load balancers, and DPI devices that make decisions based on the packet header too, because these need the ability to perform range queries. Achieving such generality provided new challenges to \sys. 
...
\todo{not generality because it is too strong}
  ...
  \todo{put the below somewehre; it is a good systems property}
