...
SP must not learn anything about $v$ other than what interval it matches to, which is the desired functionality. 
...
For example, if $v$ is 127.0.0.1, a possible encryption is 48.124.24.85. The encryption does not retain any information about $v$ other than the range it is in. In particular, for two values $v_1$ and $v_2$ that match the same interval, their order can be arbitrary. Thus, this satisfies the security requirement above.
...
The same problem occurs if packets encrypted under the new tree arrive before the new ruleset has been adopted.

To avoid these problems, when the RangeMatch tree is updated, the gateway and cloud provider do the following. 
