...
\begin{figure}[t]
  ...
  \caption[]{RangeMatch encryption for two ranges, $R1$ and $R2$.\label{fig:rangeopts}}
\end{figure}
To generalize this insight, we need to specify how to encrypt $v$ when $v$ fits in multiple intervals or in no interval at all. Figure~\ref{fig:rangeopts} illustrates how $v$ is mapped depending on whether it maps to zero, one, or both of two ranges, $R1$ and $R2$; we formalize this as follows. 

To achieve the desired security, the we find interval $I$ (which may consist of many sub-intervals, when $v$ matches neither $R1$ or $R2$ in Fig.~\ref{fig:rangeopts}) inside which $v$ should be mapped at random. The equation for $I$ is as follows. Consider the intervals {\em inside} which $v$ falls, and let $I_0, I_1, \dots, I_{n_I}$ be their encryptions. We always include the total interval $I_0 = [0.0.0.0, 255.255.255.255]$. Now consider the intervals {\em outside} of which $v$ falls and let $O_1, \dots, O_{n_O}$ be their encryptions. Then, to provide our desired security goal, $v$ should be chosen at random from the interval  
\begin{equation}
...
\subsubsection{Implementing RangeMatch}
\label{sec:tree}
