...
some service. The enterprise runs a gateway (GW) which sends traffic to middlebox (MB) running in the cloud; in practice this cloud may be either a public cloud service (such as EC2), or an ISP-supported service running at a Central Office (CO).
...
\todo{for Justine: add here an explanation and reference to eval why this is not a big deal}
...
%Before defining our threat model precisely, we first provide context and background on the concerns clients have about cloud providers today.
  Clients adopt cloud services for decreased cost and ease of management  because the providers are known and trusted to provide good service. 
...
The key idea is to encrypt packets {\it field-by-field}: \eg{}  an encrypted packet will contain a source address that is an encryption of the original packet's source address. We ensured that the encryption has the same size as the original data, and placed any additional encrypted information or metadata in the options field of a packet. 
...
For example, we encrypt an IP address using RangeMatch. This allows, e.g., a firewall to check whether the packet's source belongs to a prefix known to be controlled by a botnet -- but without learning what the actual source IP address is.
...
As mentioned, when compared to BlindBox, \sys provides new cryptography and system design that enable it to achieve generality --  it turns out that, on top of these, the architectural setting of \sys provides additional benefits, which we can now explain.
...
However, in the \sys context, this exchange can be performed just once at the gateway because the connection between the gateway and the cloud provider is long-lived. Moreover, there is no overhead per user connection. 
...
The second benefit is increased deployability. In \sys, the gateway does the encryption, so deployability improves because the end hosts do not need to be modified.
