...
However, in the \sys context, this exchange can be performed just once at the gateway because the connection between the gateway and the cloud provider is long-lived. Consequently, there is no overhead per user connection. 
...
The second benefit is increased deployability. In \sys, the gateway performs the encrtyption rather than the endhosts, as in BlindBox. Hence, deployability improves because the end hosts do not need to be modified.
