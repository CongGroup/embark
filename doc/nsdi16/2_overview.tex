...
%% Reviewer comes in thinking, "But what about BlindBox?" We have them know we beat BB on page 2.
%% Pagees 3-12, they've forgotten all about it now and are enamored with RM. Win!

The architecture we have introduced so far is a conceptually simple change from the deployment scenario for BlindBox.
BlindBox focuses on an end host communicating directly with a middlebox, but \sys allows many end hosts to push the cost of encryption to a shared gateway.
This modification alone, if applied directly to BlindBox, takes BlindBox from impractical for deployment to practice.

BlindBox requires 97 seconds to perform the exchange 
