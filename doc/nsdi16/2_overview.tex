...
%Before defining our threat model precisely, we first provide context and background on the concerns clients have about cloud providers today.
  ...
  Reports in the popular press describe companies selling customer data to marketers~\cite{radioshack}, disgruntled employees snooping or exporting data~\cite{att}, and hackers gaining access to clouds~\cite{somethingelse}.
...
We discuss how rules are encrypted rules are generated and distributed given these different trust settings in \S\ref{sec:rules}.
...
\justine{mention deterministic enc here?}
\begin{myitemize}
...
Our goal with \sys is to design an approach to outsource {\it all} enterprise middleboxes to the cloud.
However it is worth noting that the very simple change in setting from BlindBox to \sys makes encrypted processing for DPI practical for usage where BlindBox remains impractical.

BlindBox requires 97 seconds to perform the exchange in which rules are encrypted so that they may be compared against encrypted traffic.
In the original BlindBox setting, this exchange must be performed for every new connection since each session is between two new endhosts; this requires a new, ephemeral key to be used between client and destination.
However, in the \sys context, this exchange can be performed once at the gateway, after which the same rules can be reused for all clients; this key is safe to re-use becuase it is private to the gateway.
%%Not sure if the above made sense.

The change in context has two further implications in deployability and security. 
Deployability improves as the end hosts do not need to be modified -- the gateway performs all encryption -- resulting in improved deployability properties. 

Security improves because the rule exchange is a one-time cost we can encrypt more rules during the setup phase without incurring a per-connection overhead.
We take advantage of this in the following way.
BlindBox performs detection directly over encrypted traffic {\it only} for IDS rules which involve exact match strings; when regular expressions are involved in the rulset BlindBox must resort to decrypting the traffic under certain circumstances.
Many regular expressions can be converted to a set of exact-match strings, for example /hello[1-3]/ is equivalent to exact matches on "hello1", "hello2", "hello3".
We can convert many regular expressions to exact matches in this way, reducing the amount of data that must be decrypted by BlindBox.
However, many regular expressions remain too complex to do so -- if the set of potential exact matches is large, we must leave it as a regular expression and resort to decryption.
As we will show in \S\ref{sec:eval}, this approach halves the number of rules that require decryption to fully evaluate given a 2GB memory budget for exact-match terms.

In the rest of this paper, we focus not on improving the capabilities of BlindBox, but on new capabilities that will allow us to outsource middleboxes {\it generally.}
