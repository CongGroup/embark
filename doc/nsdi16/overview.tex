...
{\bf Context.} 
  ...
  This type of threat is referred to as an `honest but curious'~\cite{honestbutcurious} or `passive'~\cite{passiveattack} attacker: a party who is trusted to handle the data and deliver or service it correctly, but who may want to learn more from the data than is strictly necessary or desired.
  This type of attacker differs from a classical, `active' attacker, who seeks to harm the client outright, for example by manipulating data or denying service~\cite{activeattacks}.
