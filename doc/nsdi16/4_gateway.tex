...
Normal KeywordMatch permits comparison between encrypted values and rules, but not between one value and another value; deterministic KeywordMatch permits two values to be compared as well because of the lack of randomization.
...
\todo{need to explain why deterministic is not too bad in security for web proxies -- this is the functionality we want; also the 
pages at the proxy will be distinct, but of course you can tell which packets contain this URL; I would add that this is
the only place where we use this enc schemes}
