...
If the client is not permitted to know the rules, we must somehow allow the cloud provider (who knows the rules, but should not learn the client's key) to learn the encryption of each rule with the client's key. This is achieved using a classical combination of Yao's garbled circuits~\cite{Yao86} with oblivious transfer~\cite{Naor-Pinkas}, as originally applied by BlindBox (where the client may never learn the rules)~\cite{blindbox}.
