...
#include "threadsafeencoderelement.hh"
...
ThreadSafeEncoderElement::ThreadSafeEncoderElement(){
  //_lk = 0;
  mcs_lock_init(&_lk);
...
int ThreadSafeEncoderElement::configure(Vector<String> &conf, ErrorHandler *errh){
...
void ThreadSafeEncoderElement::push(int, Packet *p){
  ...
  if(a == NULL){
    ...
    a->_parent_thread = p->_parent_thread;
    output(0).push(a);
    p->kill();
  }
...
inline unsigned char ThreadSafeEncoderElement::sigmask_isAGoodHash(uint64_t hash) {
  return((hash & sigmask) == 0);
}

unsigned short ThreadSafeEncoderElement::walk_left(
...
unsigned short ThreadSafeEncoderElement::walk_right(
    const unsigned char *pStore,
    const unsigned char *pHere,
    unsigned short redundancy_limit)
{
...
void inline ThreadSafeEncoderElement::hash_advanceHash(uint64_t *pHash,
...
int inline ThreadSafeEncoderElement::hash_testAndInsert(uint64_t hash, pkt *pPkt, unsigned short offset, unsigned short *pRedundantLeftBound,int inserts)
{
...
void ThreadSafeEncoderElement::collision_detect(hashEntry *pHashEntry,
...
inline void ThreadSafeEncoderElement::hash_testAndNoCollide(uint64_t hash,  unsigned short offset)
{
  hashEntry *pHashEntry = index->lookup(hash, max_packets, current_uuid);
...
Packet *ThreadSafeEncoderElement::computeHash(Packet *p){
  /* 
  ...
  //Justine: Still haven't touched any cross core state until below!
  else{
    struct _mcslock_node my_lock_node;
    //while(_lk.compare_swap(0, 1) != 0) _mm_pause();
      mcs_lock(&_lk, &my_lock_node);
...
void ThreadSafeEncoderElement::setupTable(){
  uint64_t pn = 1;
  unsigned int i,j;
  for(j=0; j<siglen; j++){
...
uint64_t ThreadSafeEncoderElement::hash_getmaskoflength(unsigned int new_masklen) {
  uint64_t mask;
  if(new_masklen < 64) {
...
EXPORT_ELEMENT(ThreadSafeEncoderElement)
ELEMENT_MT_SAFE(ThreadSafeEncoderElement)
