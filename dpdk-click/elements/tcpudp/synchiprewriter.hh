...
class SynchIPRewriter : public TCPRewriter { public:
  ...
  void acquire_map_lock(Packet* p){
    if(global_replay_timestep >= max_global_replay_timestep){
      ...
      if(p->ip_header()->ip_p == IP_PROTO_TCP){
        WritablePacket* newpkt = Packet::make_log_pkt(cur_virt_timestep, p->_pd_batch_size, global_tid, p->tcp_header()->th_dport, p->dst_ip_anno().addr());
        output(2).push(newpkt);
      }else if(p->ip_header()->ip_p == IP_PROTO_UDP){
         WritablePacket* newpkt = Packet::make_log_pkt(cur_virt_timestep, p->_pd_batch_size, global_tid, p->udp_header()->uh_dport, p->dst_ip_anno().addr());
        output(2).push(newpkt);
      }
      ...
      //std::cout << (int) global_tid << " trying to grab lock! () Allowed: " << (int) next_thread << std::endl;
