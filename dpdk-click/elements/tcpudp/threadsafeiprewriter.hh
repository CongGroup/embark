...
/*
   ...
   ThreadSafeIPRewriter(INPUTSPEC1, ..., INPUTSPECn [, I<keywords>])
   ...
   ThreadSafeIPRewriter implements the functionality of a network address/port translator
   ...
   ThreadSafeIPRewriter maintains a I<mapping table> that records how packets are
   ...
   When ThreadSafeIPRewriter receives a packet, it first looks up that packet in the
...
address and port unchanged.  SPORT may be a port range 'L-H'; ThreadSafeIPRewriter will
...
ThreadSafeIPRewriterPatterns element.
...
ThreadSafeIPRewriter has no mappings when first initialized.
...
on a 'pass' input port.  ThreadSafeIPRewriter changes IP packet data and, optionally,
   ...
   dropped if an ThreadSafeIPRewriter's mapping table is full of guaranteed flows.
   ...
   Returns the number of mappings in this ThreadSafeIPRewriter's mapping table.

   =h mapping_failures r

   Returns the number of mapping failures, which can occur, for example, when the
   ThreadSafeIPRewriter runs out of source ports, or when a new flow is dropped because the
   ThreadSafeIPRewriter is full.
   ...
   Returns a human-readable description of the ThreadSafeIPRewriter's current set of TCP
   ...
   Returns a human-readable description of the ThreadSafeIPRewriter's current set of
   UDP mappings.

   =a TCPRewriter, IPAddrRewriter, IPAddrPairRewriter, ThreadSafeIPRewriterPatterns,
   RoundRobinIPMapper, FTPPortMapper, ICMPRewriter, ICMPPingRewriter */

   class ThreadSafeIPRewriter : public TCPRewriter { public:

     typedef UDPRewriter::UDPFlow UDPFlow;

     ThreadSafeIPRewriter();
     ~ThreadSafeIPRewriter();

     const char *class_name() const		{ return "ThreadSafeIPRewriter"; }
     ...
     static inline Map &reply_udp_map(IPRewriterInput *rwinput) {
       ThreadSafeIPRewriter *x = static_cast<ThreadSafeIPRewriter *>(rwinput->reply_element);
       return x->_udp_map;
     }
  ...
  inline void
ThreadSafeIPRewriter::destroy_flow(IPRewriterFlow *flow)
{
  if (flow->ip_p() == IP_PROTO_TCP)
