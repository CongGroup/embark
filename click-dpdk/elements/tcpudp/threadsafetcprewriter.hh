#ifndef CLICK_ThreadSafeTCPRewriter_HH
#define CLICK_ThreadSafeTCPRewriter_HH
...
/*
   ...
   ThreadSafeTCPRewriter(INPUTSPEC1, ..., INPUTSPECn [, KEYWORDS])
   ...
   In addition to IPRewriter's functionality, the ThreadSafeTCPRewriter element can add or
   ...
   dropped if an ThreadSafeTCPRewriter's mapping table is full of guaranteed flows.
...
Returns a human-readable description of the ThreadSafeTCPRewriter's current set of
mappings.

=a IPRewriter, IPAddrRewriter, IPAddrPairRewriter, IPRewriterPatterns,
  FTPPortMapper */

  class ThreadSafeTCPRewriter : public IPRewriterBase { public:
    ...
    const char *class_name() const		{ return "ThreadSafeTCPRewriter"; }
  ...
  inline void
ThreadSafeTCPRewriter::destroy_flow(IPRewriterFlow *flow)
{
...
inline tcp_seq_t
ThreadSafeTCPRewriter::TCPFlow::new_seq(bool direction, tcp_seq_t seqno) const
{
...
inline tcp_seq_t
ThreadSafeTCPRewriter::TCPFlow::new_ack(bool direction, tcp_seq_t ackno) const
{
  delta_transition *dt = _dt;
  while (dt && dt->has_trigger(!direction)
