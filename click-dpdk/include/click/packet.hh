...
#if CLICK_USERLEVEL
inline WritablePacket *
Packet::make(unsigned char *data, uint32_t length, unsigned char *ptr_head, uint16_t buf_len,
...
{                                       
  ...
  if (p) {                                                                                         
    p->initialize();                                                                             
    p->_has_rte_mbuf = has_rte_mbuf;                                                             
    p->_head = ptr_head;                                                                         
    //getting the mbuf address of the packet                                                     
    p->ptr_mbuf = RTE_MBUF_FROM_BADDR(ptr_head);                                                 
    p->_data = data;                                                                             
    //there is no current specific/extra tailroom for rte_mbufs                                  
    //only what is left beyond pkt_len up to buf_len                                             
    p->_tail = data + length;                                                                    
    p->_end = ptr_head + buf_len;                                                                
...
{
#if CLICK_USERLEVEL
  //maz: freeing rte_mbuf, this pkt contains rte_mbuf                                  
  if (this->_has_rte_mbuf)
    rte_pktmbuf_free(this->ptr_mbuf);
#endif
#if CLICK_LINUXMODULE
...
Packet::push(uint32_t len)
{
	...
	q->_data -= len;
	//check to see if this is rte_mbuf, must later take of expensive push
	if (likely(_has_rte_mbuf))
	  rte_pktmbuf_prepend(ptr_mbuf, len);
# if CLICK_BSDMODULE
	q->m()->m_data -= len;
	q->m()->m_len += len;
...
#else				/* User-space and BSD kernel module */
	q->_tail += len;
	//check for rte_mbuf, NOTE: ONLY supporting one segment pkts
	if (likely(_has_rte_mbuf))
	  rte_pktmbuf_append(ptr_mbuf, len);
...
#else				/* User-space and BSD kernel module */
    _tail -= len;
    //check for rte_mbuf, NOTE: only supporting 1 segment pkts
    if (likely(_has_rte_mbuf))
      rte_pktmbuf_trim(ptr_mbuf, len);
